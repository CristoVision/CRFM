...
import { formatTime, parseLrcTextToLines, generateLrcContent, getStoragePath } from '../lrcEditorUtils.js';
...
import MobileSyncControls from '@/components/lrcEditorComponents/MobileSyncControls';
    ...
    function LrcEditorModal({ isOpen, onOpenChange, track, allTracks, currentTrackIndexInList, onLyricsUpdated, onSwitchTrack }) {
      const { user } = useAuth();
      ...
      const loadLyrics = useCallback(async (currentTrack) => {
        ...
        if (storagePath) {
          try {
            const { data, error } = await supabase.storage.from('lyrics-sync-files').download(storagePath);
            if (error) {
        ...
        const parsedLines = parseLrcTextToLines(lrcTextContent, !fromStorage); // Pass true to filter metadata if not from .lrc file
      ...
      useEffect(() => {
        ...
        if (audio) {
          ...
          return () => {
            ...
            // Safe to remove listeners even if they weren't added if audio becomes null
            audio.removeEventListener('play', () => setIsPlaying(true));
            audio.removeEventListener('pause', () => setIsPlaying(false));
            audio.removeEventListener('ended', () => setIsPlaying(false));
          };
        }
      }, [audioRef.current]); // Re-run if audioRef.current itself changes
      ...
      const performSave = async (currentTrackToSave) => {
        ...
        // Always generate a new path for saving to ensure it's a new version or fresh save
        let newLrcStoragePath = `public/${currentTrackToSave.id}_${Date.now()}.lrc`;
        let fileToUpload;

        if (lrcFile) { // If a new file was explicitly uploaded by user
          lrcContentToSave = await lrcFile.text(); // Use content from this file
          newLrcStoragePath = `public/${currentTrackToSave.id}_${Date.now()}_uploaded.lrc`; // Ensure distinct name for uploaded
          fileToUpload = lrcFile;
        } else { // Standard save operation (edited lyrics or first time save)
        ...
        try {
          const { error: uploadError } = await supabase.storage
            ...
            .upload(newLrcStoragePath, fileToUpload, { upsert: true }); // upsert true means it will overwrite if path exists
          if (uploadError) throw uploadError;
          
          const { error: updateError } = await supabase
            .from('tracks')
            .update({ lrc_file_path: newLrcStoragePath, lyrics_text: lrcContentToSave }) 
            .eq('id', currentTrackToSave.id)
            .eq('uploader_id', user.id);
          if (updateError) throw updateError;

          toast({ title: "LRC Saved!", description: `Lyrics for ${currentTrackToSave.title} saved.` });
          onLyricsUpdated({ ...currentTrackToSave, lrc_file_path: newLrcStoragePath, lyrics_text: lrcContentToSave });
        ...
        setLrcFile(file);
         if (file) {
            ...
            reader.onload = (e) => {
                ...
                const parsedLines = parseLrcTextToLines(content, false); // false: don't filter metadata from uploaded .lrc
                setLyrics(parsedLines);
                pushToHistory(parsedLines, true); 
      ...
      const handleSaveUploadedLrc = () => performSave(track); // This will now use the lrcFile state
      ...
      useEffect(() => {
        const handleGlobalKeyDown = (event) => {
          ...
          if (lyricsPanelRef.current && lyricsPanelRef.current.contains(event.target)) {
            // Let LyricsPanel handle its own key events if focus is inside it
            return;
          }
      ...
      }, [isOpen, showUnsavedChangesDialog, syncLine, backLine, lyricsPanelRef, lyrics]);
      ...
      return (
        ...
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
          <DialogContent className="max-w-4xl xl:max-w-5xl w-[95vw] h-[90vh] glass-effect-light text-white flex flex-col p-0">
            ...
            {/* Footer is removed as close button is in header */}
          </DialogContent>
        </Dialog>
        ...
        <AlertDialog open={showUnsavedChangesDialog} onOpenChange={setShowUnsavedChangesDialog}>
            <AlertDialogContent className="glass-effect-light text-white">
                <AlertDialogHeader>
                    <AlertDialogTitle className="flex items-center"><AlertTriangle className="text-yellow-400 mr-2" />Unsaved Changes</AlertDialogTitle>
                    <AlertDialogDescription className="text-gray-300">
                        You have unsaved changes. Do you want to save them before proceeding?
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                    <AlertDialogCancel onClick={() => confirmUnsavedChanges(false)} className="bg-gray-600 hover:bg-gray-500 border-gray-500 text-white">Discard</AlertDialogCancel>
                    <AlertDialogAction onClick={() => confirmUnsavedChanges(true)} className="golden-gradient text-black">Save Changes</AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
        </>
