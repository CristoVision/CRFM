...
const EmbedPlayerSpinner = ({ label }) => (
...
const EmbedPlayerError = ({ message, title }) => (
...
function EmbedCreatorPage() {
          ...
          try {
            ...
            if (profileError) {
              if (profileError.code === 'PGRST116' || profileError.details?.includes('0 rows')) {
                throw new Error(t('embed.creator.unavailable'));
              }
              throw profileError;
            }
            if (!profile) throw new Error(t('embed.creator.unavailable'));
            setCreatorData(profile);
            document.title = `${profile.username || t('embed.creator.defaultName')} - ${t('embed.creator.embedLabel')}`;
            ...
            if (err.message.includes("Failed to fetch") || err.status === 400) {
              setError(t('embed.creator.unavailableShort'));
            } else {
              setError(err.message || t('embed.creator.unexpectedError'));
            }
            document.title = t('embed.creator.errorTitle');
          } finally {
            setLoading(false);
          }
        };
      ...
      if (loading) return <EmbedPlayerSpinner label={t('embed.creator.loading')} />;
      if (error) return <EmbedPlayerError message={error} title={t('embed.creator.errorHeading')} />;
      if (!creatorData) return <EmbedPlayerError message={t('embed.creator.dataNotFound')} title={t('embed.creator.errorHeading')} />;
      ...
      return (
        <>
          <Helmet>
            <title>{`${creatorData.username || t('embed.creator.defaultName')} - CRFM ${t('embed.creator.embedLabel')}`}</title>
            <meta name="description" content={t('embed.creator.metaDescription', { name: creatorData.username || t('embed.creator.defaultName') })} />
            <meta property="og:title" content={`${creatorData.username || t('embed.creator.defaultName')}`} />
            <meta property="og:description" content={t('embed.creator.ogDescription', { name: creatorData.username || t('embed.creator.defaultName') })} />
          ...
          <div className="w-full h-screen flex flex-col bg-gradient-to-br from-slate-800 to-slate-900 text-white font-sans overflow-hidden">
            <div className="flex items-center p-3 space-x-3 flex-shrink-0 border-b border-white/10">
              ...
              <Button variant="ghost" size="icon" asChild className="text-gray-300 hover:text-yellow-400">
                <RouterLink to={`/creator/${creatorData.id}`} target="_blank" title={t('embed.creator.viewProfileTitle')}>
            ...
            <ScrollArea className="flex-grow p-3">
              {!hasContent && (
                <p className="text-center text-sm text-gray-500 py-4">{t('embed.creator.noContent')}</p>
              )}
              {tracks.length > 0 && (
                <div className="mb-3">
                  <h3 className="text-xs font-semibold text-yellow-400 mb-1 uppercase">{t('embed.creator.latestTracks')}</h3>
              ...
              {albums.length > 0 && (
                <div className="mb-3">
                  <h3 className="text-xs font-semibold text-green-400 mb-1 uppercase">{t('embed.creator.latestAlbums')}</h3>
                  <div className="space-y-1">
              ...
              {playlists.length > 0 && (
                <div>
                  <h3 className="text-xs font-semibold text-purple-400 mb-1 uppercase">{t('embed.creator.latestPlaylists')}</h3>
                  <div className="space-y-1">
            ...
            <div className="text-center py-2 border-t border-white/10 flex-shrink-0 flex items-center justify-center space-x-2">
              <RouterLink to={`/creator/${creatorData.id}`} target="_blank" className="text-xs text-yellow-400 hover:text-yellow-300 hover:underline">
                {t('embed.creator.viewFullProfile')}
              </RouterLink>
              <RouterLink to="/" target="_blank" title={t('embed.common.poweredBy')}>
                <img src={CRFM_LOGO_URL} alt={t('embed.common.crfmLogoAlt')} className="w-4 h-4 opacity-70 hover:opacity-100 transition-opacity" />
              </RouterLink>
            </div>
          </div>
        </>
