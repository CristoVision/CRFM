...
import CoverArtMedia from '@/components/common/CoverArtMedia';
    ...
    const EmbedPlaylistPage = () => {
      ...
      if (!playlist) {
         return (
          <div className="w-full h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-900 to-gray-800 text-white p-4">
            ...
            <p className="text-lg">Playlist data not found.</p>
          </div>
        );
      }
      ...
      return (
        <>
        <Helmet>
            <title>{playlist ? `${playlist.title} by ${playlist.profiles?.username || 'CRFM User'}` : "Embedded Playlist"} - CRFM</title>
            <meta name="description" content={playlist ? `Listen to the playlist ${playlist.title} embedded from CRFM.` : "Embedded music playlist player from CRFM."} />
            {playlist && <meta property="og:title" content={`${playlist.title} - ${playlist.profiles?.username || 'CRFM User'}`} />}
            {playlist && <meta property="og:description" content={`Listen to the playlist ${playlist.title} on CRFM.`} />}
        ...
        <div className={`w-full ${isFullScreen ? 'h-screen' : 'max-w-md mx-auto h-[280px] sm:h-[240px]'} flex flex-col bg-gradient-to-br from-slate-800 to-gray-900 text-white shadow-2xl rounded-lg overflow-hidden transition-all duration-300`}>
          <div className="flex items-center p-3 space-x-3">
            ...
            <div className="flex flex-col justify-between flex-grow min-w-0 h-full">
              <div>
                ...
                <p className="text-xs sm:text-sm text-gray-400 truncate" title={playlist.profiles?.username || 'CRFM User'}>By: {playlist.profiles?.username || 'CRFM User'}</p>
          ...
          <div className="bg-black/30 px-3 py-1.5 flex justify-between items-center text-xs">
            ...
            <div className="flex items-center space-x-1">
              ...
              <Button variant="ghost" size="icon" onClick={() => setShowTracklist(!showTracklist)} className="text-gray-400 hover:text-white w-6 h-6" title={showTracklist ? "Hide Tracklist" : "Show Tracklist"} disabled={tracks.length === 0}>
