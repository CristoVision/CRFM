    ...
    const VideoDetailPage = () => {
      ...
      useEffect(() => {
        const fetchVideo = async () => {
          ...
          try {
            ...
            setVideo({
              ...data,
              creator_display_name: data.profiles?.username || t('playlistDetail.labels.unknownCreator'),
            ...
            toast({ title: t('videoDetail.toasts.errorTitle'), description: t('videoDetail.toasts.loadFailed', { message: err.message }), variant: "destructive" });
          } finally {
            setLoading(false);
          }
        };
      ...
      const handleToggleFavorite = async () => {
        ...
        if (typeof addFavorite !== 'function' || typeof removeFavorite !== 'function') {
          toast({ title: t('videoDetail.toasts.featureUnavailableTitle'), description: t('videoDetail.toasts.featureUnavailableBody'), variant: "destructive" });
          return;
        }
        ...
        try {
          if (isFavorited) {
            ...
            toast({ title: t('videoDetail.toasts.removedFavorite'), variant: "success" });
          } else {
            await addFavorite('video', video.id);
            toast({ title: t('videoDetail.toasts.addedFavorite'), variant: "success" });
          }
        } catch (err) {
          toast({ title: t('videoDetail.toasts.favoriteError'), description: err.message, variant: "destructive" });
        } finally {
      ...
      const handlePlayVideo = async () => {
        ...
        if (cost > 0 && typeof spendCrossCoinsForVideo === 'function') {
          ...
          if (!success) {
            toast({
              title: t('videoDetail.toasts.cannotPlayTitle'),
              description: error || t('videoDetail.toasts.cannotPlayBody', { cost }),
              variant: "destructive",
            });
      ...
      if (error || !video) {
        return (
          <div className="min-h-screen gradient-bg flex flex-col items-center justify-center text-white p-4">
            ...
            <h1 className="text-3xl font-bold mb-2">{t('videoDetail.notFoundHeading')}</h1>
            <p className="text-red-300 text-center mb-6">{error || t('videoDetail.notFoundBody')}</p>
            <Button onClick={() => navigate('/')} className="golden-gradient text-black">
              {t('videoDetail.goHome')}
            </Button>
          </div>
        );
      }
      ...
      return (
        <>
          <Helmet>
            <title>{t('videoDetail.metaTitle', { title: video?.title || t('videoDetail.musicVideo') })}</title>
            <meta name="description" content={video?.description?.substring(0, 160) || t('videoDetail.metaDescription', { title: video?.title || '' })} />
          </Helmet>
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            <div className="container mx-auto max-w-5xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                >
                  <div>
                    <h1 className="text-3xl font-bold golden-text break-words">{video.title || t('videoDetail.untitled')}</h1>
                    <Badge variant="secondary" className="mt-2 bg-yellow-500 text-black text-xs font-semibold">
                      {t('videoDetail.musicVideo')}
                    </Badge>
                  </div>
                  ...
                  <div className="flex items-center space-x-3 cursor-pointer" onClick={() => video.creator_id && navigate(`/creator/${video.creator_id}`)}>
                    ...
                    <div>
                      <p className="text-sm text-gray-400">{t('videoDetail.createdBy')}</p>
                  ...
                  <div className="flex space-x-3">
                    ...
                    >
                      ...
                      {isFavorited ? t('videoDetail.favorited') : t('videoDetail.addFavorite')}
                    </Button>
                    <Button 
                    ...
                    >
                      <Share2 className="w-4 h-4 mr-2" /> {t('videoDetail.share')}
                    </Button>
                  </div>
                  ...
                  {!user && (
                     <div className="mt-2 text-xs text-orange-400 flex items-center justify-center p-3 bg-orange-500/10 rounded-md border border-orange-500/30">
                        ...
                        <span>{t('videoDetail.loginRequired')}</span>
                    </div>
                  )}
                  ...
                  {video.cost_cc > 0 && (
                    <div className="flex items-center text-yellow-400 p-3 bg-yellow-500/10 rounded-md border border-yellow-500/30">
                      ...
                      <span className="text-sm">{t('videoDetail.costToWatch', { cost: video.cost_cc, suffix: video.cost_cc !== 1 ? 's' : '' })}</span>
                    </div>
                  )}
                </motion.div>
              </div>
              ...
              >
                <h2 className="text-2xl font-semibold mb-4 text-slate-100">{t('videoDetail.detailsTitle')}</h2>
                <div className="space-y-3 text-sm">
                  <div className="flex items-start">
                    <strong className="w-32 text-gray-400 shrink-0 flex items-center"><User className="w-4 h-4 mr-2 text-yellow-400/70"/>{t('videoDetail.labels.artist')}</strong>
                    <span className="text-gray-200">{video.creator_display_name}</span>
                  </div>
                  <div className="flex items-start">
                    <strong className="w-32 text-gray-400 shrink-0 flex items-center"><Calendar className="w-4 h-4 mr-2 text-yellow-400/70"/>{t('videoDetail.labels.published')}</strong>
                    <span className="text-gray-200">{new Date(video.created_at).toLocaleDateString(language === 'es' ? 'es-ES' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                  </div>
                   {video.language && (
                      <div className="flex items-start">
                        <strong className="w-32 text-gray-400 shrink-0 flex items-center"><LanguagesIcon className="w-4 h-4 mr-2 text-yellow-400/70"/>{t('videoDetail.labels.language')}</strong>
                        <span className="text-gray-200">{video.language}</span>
                      </div>
                    )}
                  <div className="flex items-start">
                    <strong className="w-32 text-gray-400 shrink-0 flex items-center"><Tag className="w-4 h-4 mr-2 text-yellow-400/70"/>{t('videoDetail.labels.description')}</strong>
                    <p className="text-gray-300 whitespace-pre-wrap leading-relaxed">{video.description || t('videoDetail.labels.noDescription')}</p>
                  </div>
                </div>
              </motion.div>
