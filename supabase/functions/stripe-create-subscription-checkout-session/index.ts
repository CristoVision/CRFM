...
function resolvePriceId(plan: Plan) {
  if (plan === 'monthly') return Deno.env.get('STRIPE_PRICE_UNLIMITED_MONTHLY');
  if (plan === 'six_months') return Deno.env.get('STRIPE_PRICE_UNLIMITED_6MO');
  return Deno.env.get('STRIPE_PRICE_UNLIMITED_YEARLY');
}
