...
begin
    if not exists (
        ...
        where polname = 'wallet_action_requests_select_self_or_admin'
    ) then
    ...
    if not exists (
        select 1 from pg_policies
        where polname = 'wallet_action_requests_insert_self'
    ) then
    ...
    if not exists (
        select 1 from pg_policies
        where polname = 'wallet_action_requests_admin_update'
    ) then
...
begin
    if not exists (
        select 1 from pg_policies
        where polname = 'wallet_methods_select_any'
    ) then
    ...
    if not exists (
        select 1 from pg_policies
        where polname = 'wallet_methods_admin_insert'
    ) then
    ...
    if not exists (
        select 1 from pg_policies
        where polname = 'wallet_methods_admin_update'
    ) then
...
begin
    if not exists (
        select 1 from pg_policies
        where polname = 'wallet_redeem_codes_admin_select'
    ) then
    ...
    if not exists (
        select 1 from pg_policies
        where polname = 'wallet_redeem_codes_admin_insert'
    ) then
    ...
    if not exists (
        select 1 from pg_policies
        where polname = 'wallet_redeem_codes_admin_update'
    ) then
